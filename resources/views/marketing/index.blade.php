...
@section('title', 'Penawaran Marketing')

@section('content')
<div class="p-6">
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-white">Penawaran Marketing</h1>
            <p class="text-gray-400 text-sm">Catat dan update progress penawaran untuk Web, Internet, dan CCTV.</p>
        </div>
        <a href="{{ route('marketing.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition text-sm font-medium">
            Tambah Penawaran
        </a>
    </div>
    ...
    <form method="GET" action="{{ route('marketing.index') }}" class="bg-gray-800 rounded-lg border border-gray-700 p-4 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('marketing.index') }}" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition">Reset</a>
            </div>
        </div>
    </form>
                    ...
                    @forelse($offers as $offer)
                        ...
                        <tr class="hover:bg-gray-700/30 transition">
                            <td class="px-4 py-3">
                                ...
                                <p class="text-xs text-gray-400">{{ $offer->contact_person }} - {{ $offer->contact_position }}</p>
