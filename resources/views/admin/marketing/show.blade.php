...
<div class="p-6 max-w-6xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('admin.marketing.index') }}" class="text-blue-400 hover:underline text-sm">Kembali ke Laporan Marketing</a>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div><p class="text-gray-500">Nama PIC</p><p class="text-white font-medium">{{ $offer->contact_person ?: '-' }}</p></div>
                    <div><p class="text-gray-500">Jabatan PIC</p><p class="text-white font-medium">{{ $offer->contact_position ?: '-' }}</p></div>
                    <div><p class="text-gray-500">Nomor Kontak</p><p class="text-white font-medium">{{ $offer->contact_phone ?: '-' }}</p></div>
                    <div><p class="text-gray-500">Email</p><p class="text-white font-medium">{{ $offer->contact_email ?: '-' }}</p></div>
                    <div><p class="text-gray-500">Bidang</p><p class="text-white font-medium">{{ ucfirst($offer->category) }}</p></div>
                    <div><p class="text-gray-500">Marketing</p><p class="text-white font-medium">{{ $offer->employee?->name ?: '-' }}</p></div>
                    <div><p class="text-gray-500">Tanggal Penawaran</p><p class="text-white font-medium">{{ $offer->offer_date?->format('d/m/Y') ?? '-' }}</p></div>
                    <div><p class="text-gray-500">Tanggal Follow Up</p><p class="text-white font-medium">{{ $offer->follow_up_date?->format('d/m/Y') ?? '-' }}</p></div>
                    <div><p class="text-gray-500">Jadwal Meeting</p><p class="text-white font-medium">{{ $offer->meeting_date?->format('d/m/Y H:i') ?? '-' }}</p></div>
                    <div>
                        <p class="text-gray-500">Estimasi Nilai</p>
                        <p class="text-white font-medium">{{ $offer->estimated_value ? 'Rp ' . number_format((float) $offer->estimated_value, 0, ',', '.') : '-' }}</p>
                    </div>
                </div>
            </div>
